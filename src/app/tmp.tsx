...
export default function MedicalCoTApp() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const payload = {
          inputs: {
            prompt: `你是一位专业的医学影像讲师，请根据图像进行结构观察与教学分析，不进行医学诊断。\n请描述图像中的结构异常、特征性区域、影像对称性、密度变化等，并提出可能值得进一步检查的方向。`,
            image: base64Image
          },
          options: { wait_for_model: true }
        };

        const res = await fetch("https://api-inference.huggingface.co/models/liuhaotian/llava-v1.5-13b", {
          method: "POST",
          headers: {
        ...
        if (!res.ok) {
          ...
          setDiagnosis(`❌ HuggingFace请求失败：${res.status} ${res.statusText}\n${errorText}`);
          setReasoning("");
          setLoading(false);
          return;
        }
        ...
        const content = data.generated_text || "（无响应内容）";
      ...
      setDiagnosis(`⚠️ HuggingFace 网络或Key问题：${err.message}`);
      setReasoning("");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">医学影像推理演示平台（使用 HuggingFace LLaVA）</h1>
      <Card className="mb-4">
